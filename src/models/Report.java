...
@NamedQueries({
        ...
        //日報一覧にフォローしている人の日報のみを表示する
        @NamedQuery(name = "getFollowerReports", query = "SELECT r FROM Report As r WHERE r.employee in (SELECT f.followee FROM Follow As f WHERE f.follower = :login_employee)"),
        //※フォローしている人の日報の数を表示する
        @NamedQuery(name = "getFollowerReportsCount", query =  "SELECT COUNT(r) FROM Report As r WHERE r.employee in (SELECT f.followee FROM Follow As f WHERE f.follower = :login_employee)"),
})
@Entity
public class Report {
    @Id
